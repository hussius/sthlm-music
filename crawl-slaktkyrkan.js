...
const BASE_URL = 'https://slakthusen.se/venue/slaktkyrkan/';
const VENUE_NAME = 'Slaktkyrkan';
...
try {
  ...
  while (hasMorePages) {
    ...
    // Find all event items - they're in <a> tags within a list
    const pageEvents = [];
    ...
    $('a[href*="/"]').each((i, elem) => {
      try {
        ...
        // Only process events for Slaktkyrkan venue
        if (!stalle.toLowerCase().includes('slaktkyrkan')) {
          return;
        }
        ...
        pageEvents.push({
          name: titel,
          artist: titel.split('|')[0].trim(), // Split on | to get artist name
          venue: VENUE_NAME,
          date: eventDate,
          time: '20:00',
          ...
          sourceId: `slaktkyrkan-${titel}-${eventDate.toISOString().split('T')[0]}`,
